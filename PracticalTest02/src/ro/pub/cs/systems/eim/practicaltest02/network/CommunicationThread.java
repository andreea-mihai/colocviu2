...
//							params.add(new BasicNameValuePair(Constants.QUERY_ATTRIBUTE, city));
							...
							HttpGet httpPost = new HttpGet(Constants.WEB_SERVICE_ADDRESS + "?query="+city );
							...
							if (pageSourceCode != null) {
								...
								for (Element script : scripts) {
									...
									if (scriptData.contains(Constants.SEARCH_KEY)) {
										int position = scriptData.indexOf(Constants.SEARCH_KEY)
												+ Constants.SEARCH_KEY.length();
										scriptData = scriptData.substring(position);

										JSONObject content = new JSONObject(scriptData);

										JSONObject currentObservation = content
												.getJSONObject(Constants.CURRENT_OBSERVATION);
										...
										weatherForecastInformation = new WeatherForecastInformation(temperature,
												windSpeed, condition, pressure, humidity);
